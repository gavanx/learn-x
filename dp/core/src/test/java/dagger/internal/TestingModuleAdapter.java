...
public class TestingModuleAdapter<M> extends ModuleAdapter<M> {
    public TestingModuleAdapter(Class<M> moduleClass, Module annotation) {
        super(moduleClass, injectableTypesToKeys(annotation.injects()), annotation.staticInjections(), annotation.overrides(), annotation.includes
            (), annotation.complete(), annotation.library());
    }
    ...
    private static String[] injectableTypesToKeys(Class<?>[] injectableTypes) {
        ...
        for (int i = 0; i < injectableTypes.length; i++) {
            ...
            result[i] = injectableType.isInterface() ? Keys.get(injectableType) : Keys.getMembersKey(injectableType);
        }
        return result;
    }
            ...
            for (Method method : c.getDeclaredMethods()) {
                ...
                if (provides != null) {
                    ...
                    if (Provider.class.equals(typeToCheck)) {
                        throw new IllegalStateException("@Provides method must not return Provider directly: " + c.getName() + "." + method.getName
                            ());
                    }
                    if (Lazy.class.equals(typeToCheck)) {
                        throw new IllegalStateException("@Provides method must not return Lazy directly: " + c.getName() + "." + method.getName());
                    }
    ...
    private void handleBindings(BindingsGroup bindings, M module, Method method, String key, boolean library) {
        bindings.contributeProvidesBinding(key, new ReflectiveProvidesBinding<M>(method, key, moduleClass.getName(), module, library));
    }

    private void handleSetBindings(BindingsGroup bindings, M module, Method method, String setKey, String providerKey, boolean library) {
        SetBinding.<M>add(bindings, setKey, new ReflectiveProvidesBinding<M>(method, providerKey, moduleClass.getName(), module, library));
    }
    ...
    public M newModule() {
        try {
            ...
            throw new IllegalArgumentException("Could not construct " + moduleClass.getName() + " as it lacks an accessible no-args constructor. " +
                "This module must be passed" + " in as an instance, or an accessible no-args constructor must be added.", e);
    ...
    private static final class ReflectiveProvidesBinding<T> extends ProvidesBinding<T> {
        ...
        public ReflectiveProvidesBinding(Method method, String key, String moduleClass, Object instance, boolean library) {
