...
public class AnnotationHelper {
    ...
    /**
     * This method may return null if the {@link TypeElement} cannot be found in the processor classpath
     */
    ...
    /**
     * Returns a list of {@link JFieldRef} linking to the R class, based on the given annotation
     * 
     * @see #extractAnnotationResources(Element, String, IRInnerClass, boolean)
     */
    public List<JFieldRef> extractAnnotationFieldRefs(Element element, String annotationName, IRInnerClass rInnerClass,
            boolean useElementName) {
        return extractAnnotationFieldRefs(element, annotationName, rInnerClass, useElementName, DEFAULT_FIELD_NAME_VALUE,
    ...
    public List<JFieldRef> extractAnnotationFieldRefs(Element element, String annotationName, IRInnerClass rInnerClass,
        ...
        for (String refQualifiedName : extractAnnotationResources(element, annotationName, rInnerClass, useElementName, idFieldName,
                resFieldName)) {
    ...
    /**
     * Method to handle all annotations dealing with resource ids that can be set using the value() parameter of the annotation
     * (as int or int[]), the resName() parameter of the annotation (as String or String[]), the element name.
     * 
     * @param element the annotated element
     * @param annotationName the annotation on the element
     * @param rInnerClass the R innerClass the resources belong to
     * @param useElementName Should we use a default fallback strategy that uses the element qualified name for a resource name
    ...
    public List<String> extractAnnotationResources(Element element, String annotationName, IRInnerClass rInnerClass, boolean useElementName) {
        return extractAnnotationResources(element, annotationName, rInnerClass, useElementName, DEFAULT_FIELD_NAME_VALUE,
                DEFAULT_FIELD_NAME_RESNAME);
    }

    public List<String> extractAnnotationResources(Element element, String annotationName, IRInnerClass rInnerClass,
            boolean useElementName, String idFieldName, String resFieldName) {
        ...
        /*
         * if nothing defined in the annotation value() parameter, we check for its resName() parameter
         */
        if (defaultResIdValue(values)) {
            ...
            if (defaultResName(resNames)) {
                ...
                /*
                 * The result will will contain all the resource qualified names based on the resource names in the resName()
                 * parameter
                 */
            ...
            /*
             * The result will will contain all the resource qualified names based on the integers in the value() parameter
             */
    ...
    public List<DeclaredType> extractAnnotationClassArrayParameter(Element element, Class<? extends Annotation> annotation,
            String methodName) {
