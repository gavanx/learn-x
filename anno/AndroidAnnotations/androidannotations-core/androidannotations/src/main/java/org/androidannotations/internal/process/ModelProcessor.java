...
public class ModelProcessor {
    ...
    public ProcessResult process(AnnotationElements validatedModel) throws Exception {
        ...
        /*
         * We generate top classes then inner classes, then inner classes of inner classes, etc... until there is no more classes
         * to generate.
         */
        ...
        for (AnnotationHandler annotationHandler : environment.getDecoratingHandlers()) {
            ...
            /*
             * For ancestors, the annotationHandler manipulates the annotated elements, but uses the holder for the root element
             */
            ...
            if (!ancestorAnnotatedElements.isEmpty()) {
                LOGGER.debug("Processing enclosed elements with {}: {}", annotationHandler.getClass().getSimpleName(),
            ...
            for (AnnotatedAndRootElements elements : ancestorAnnotatedElements) {
                ...
                /*
                 * Annotations coming from ancestors may be applied to root elements that are not validated, and therefore not
                 * available.
                 */
                if (holder != null) {
            ...
            for (Element annotatedElement : rootAnnotatedElements) {
                ...
                /*
                 * We do not generate code for elements belonging to abstract classes, because the generated classes are final
                 * anyway
                 */
    ...
    private <T extends GeneratedClassHolder> void processThrowing(AnnotationHandler<T> handler, Element element, T generatedClassHolder)
    ...
    private boolean generateElements(AnnotationElements validatedModel, ProcessHolder processHolder) throws Exception {
        ...
        for (GeneratingAnnotationHandler generatingAnnotationHandler : environment.getGeneratingHandlers()) {
            ...
            for (Element annotatedElement : annotatedElements) {
                /*
                 * We do not generate code for abstract classes, because the generated classes are final anyway (we do not want
                ...
                if (!isAbstractClass(annotatedElement)) {
                    if (processHolder.getGeneratedClassHolder(annotatedElement) == null) {
                        ...
                        if (typeElement.getNestingKind() == NestingKind.MEMBER
                            ...
                            GeneratedClassHolder generatedClassHolder = generatingAnnotationHandler.createGeneratedClassHolder(environment,
                                    typeElement);
            ...
            /*
             * We currently do not take into account class annotations from ancestors. We should careful design the priority rules
             * first.
             */
        }
