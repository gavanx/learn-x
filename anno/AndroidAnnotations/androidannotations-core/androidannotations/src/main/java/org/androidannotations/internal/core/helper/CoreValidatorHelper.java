...
public class CoreValidatorHelper extends IdValidatorHelper {
    ...
    private static final List<String> INVALID_PREF_METHOD_NAMES = Arrays.asList("edit", "getSharedPreferences", "clear", "getEditor",
            "apply");

    private static final List<Receiver.RegisterAt> VALID_ACTIVITY_REGISTER_AT = Arrays.asList(Receiver.RegisterAt.OnCreateOnDestroy,
            Receiver.RegisterAt.OnResumeOnPause, Receiver.RegisterAt.OnStartOnStop);
    private static final List<Receiver.RegisterAt> VALID_SERVICE_REGISTER_AT = Collections
            .singletonList(Receiver.RegisterAt.OnCreateOnDestroy);
    private static final List<Receiver.RegisterAt> VALID_FRAGMENT_REGISTER_AT = Arrays.asList(Receiver.RegisterAt.OnCreateOnDestroy,
            Receiver.RegisterAt.OnResumeOnPause, Receiver.RegisterAt.OnStartOnStop, Receiver.RegisterAt.OnAttachOnDetach);
        ...
        if (applicationClassName != null) {
            ...
            if (!typeElement.getModifiers().contains(Modifier.ABSTRACT) && !applicationClassName.equals(generatedComponentQualifiedName)) {
                if (applicationClassName.equals(componentQualifiedName)) {
                    ...
                    valid.addWarning("The component " + generatedComponentQualifiedName
    ...
    private <T extends Annotation> void checkDefaultAnnotation(ExecutableElement method, Class<T> annotationClass,
        ...
        if (defaultAnnotation != null) {
            if (!condition.correctReturnType(method.getReturnType())) {
                valid.addError(TargetAnnotationHelper.annotationName(annotationClass) + " can only be used on a method that returns a "
                        + expectedReturnType);
            }
        }
    }
    ...
    public void hasBeforeTextChangedMethodParameters(ExecutableElement executableElement, ElementValidation valid) {
        ...
        for (VariableElement parameter : parameters) {
            ...
            valid.addError("Unrecognized parameter ("
                    + parameter.toString()
        ...
        for (VariableElement parameter : parameters) {
            ...
            if (parameterType.equals(CanonicalNameConstants.SEEKBAR)) {
                if (seekBarParameterFound) {
                    valid.addError("Unrecognized parameter declaration. You can declare only one parameter of type "
            ...
            valid.addError("Unrecognized parameter '" + parameter.toString() + "'. %s signature should be "
        ...
        if (parameters.size() == 1) {
            ...
            if (!parameterType.equals(CanonicalNameConstants.SEEKBAR)) {
                valid.addError("Unrecognized parameter declaration. You can only have one parameter of type "
                        + CanonicalNameConstants.SEEKBAR + ". Try declaring " + executableElement.getSimpleName() + "("
    ...
    public void hasRightRegisterAtValueDependingOnEnclosingElement(Element element, ElementValidation valid) {
        ...
        for (Map.Entry<String, List<Receiver.RegisterAt>> validRegisterAt : validRegisterAts.entrySet()) {
            ...
            if (extendsType(enclosingElement, enclosingType) && !validRegisterAtValues.contains(registerAt)) {
                valid.addError("The parameter registerAt of @Receiver in " + enclosingType + " can only be one of the following values : "
                        + validRegisterAtValues);
            }
        }
    }
        ...
        if (childFragment) {
            ...
            if (!enclosingElementIsFragment) {
                validation.addError(element,
                        "The 'childFragmentManager' parameter only can be used if the class containing the annotated field is either subclass of "
        ...
        if (childFragment) {
            ...
            if (supportFragment != null && annotationHelper.isSubtype(enclosingElement, supportFragment)) {
                if (!methodIsAvailableIn(supportFragment, "getChildFragmentManager")) {
                    validation.addError(element,
                            "The 'childFragmentManager' parameter only can be used if the getChildFragmentManager() method is available in "
                    ...
                    && environment().getAndroidManifest().getMinSdkVersion() < 17) {
                validation.addError(element,
                        "The 'childFragmentManager' parameter only can be used if the getChildFragmentManager() method is available in "
                                + CanonicalNameConstants.FRAGMENT + " (from API 17). Increment 'minSdkVersion' or use "
