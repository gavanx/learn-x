...
public class ValidatorHelper {
    ...
    private static final List<String> VALID_PREFERENCE_CLASSES = asList(CanonicalNameConstants.PREFERENCE_ACTIVITY,
            ...
            CanonicalNameConstants.MACHINARIUS_V4_PREFERENCE_FRAGMENT, CanonicalNameConstants.SUPPORT_V7_PREFERENCE_FRAGMENTCOMPAT,
    ...
    public void enclosingElementHasOneOfAnnotations(Element element, List<Class<? extends Annotation>> validAnnotations,
            ElementValidation validation) {
        hasOneOfAnnotations(element, element.getEnclosingElement(), validAnnotations, validation);
    }

    public void hasOneOfAnnotations(Element reportElement, Element element, List<Class<? extends Annotation>> validAnnotations,
            ElementValidation validation) {
        checkAnnotations(reportElement, element, validAnnotations, true, validation);
    }

    public void doesNotHaveOneOfAnnotations(Element element, List<Class<? extends Annotation>> validAnnotations,
            ElementValidation validation) {
    ...
    private void checkAnnotations(Element reportElement, Element element, List<Class<? extends Annotation>> validAnnotations,
            boolean shouldFind, ElementValidation validation) {
        ...
        if (shouldFind != foundAnnotation) {
            ...
            validation.addError(reportElement, "%s can only be used in a " + element.getKind().toString().toLowerCase() + not
                    + " annotated with " + getFormattedValidEnhancedBeanAnnotationTypes(validAnnotations) + ".");
        }
    }
    ...
    public void extendsListOfView(Element element, ElementValidation valid) {
        ...
        if (!elementType.toString().equals(CanonicalNameConstants.LIST) && elementTypeArguments.size() == 1
            ...
            valid.invalidate();
            valid.addError("%s can only be used on a " + CanonicalNameConstants.LIST + " of elements extending "
            ...
            if (constructors.size() == 1) {
                ...
                if (!annotationHelper.isPrivate(constructor)) {
                    if (constructor.getParameters().size() > 1) {
                        valid.addError("%s annotated element should have a constructor with one parameter max, of type "
            ...
            if (typeMirror.getKind() != TypeKind.NONE) {
                ...
                if (typeString.startsWith(CanonicalNameConstants.SPARSE_ARRAY)) {
                    ...
                    if (typeArguments.size() != 1 || !annotationHelper.isSubtype(typeArguments.get(0), parcelableType)) {
                        ...
                        && !annotationHelper.isSubtype(typeMirror, serializableType) && !parcelerHelper.isParcelType(typeMirror)) {
    ...
    public void isViewPagerClassPresent(ElementValidation validation) {
        if (!isClassPresent(CanonicalNameConstants.VIEW_PAGER)) {
            validation.addError("The class " + CanonicalNameConstants.VIEW_PAGER
                    + " cannot be found. You have to include support v4 library");
        }
    }
}
