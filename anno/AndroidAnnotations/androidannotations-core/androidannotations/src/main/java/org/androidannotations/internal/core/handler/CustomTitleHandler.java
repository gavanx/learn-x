...
public class CustomTitleHandler extends BaseAnnotationHandler<EActivityHolder> {
    ...
    public void process(Element element, EActivityHolder holder) {
        ...
        JFieldRef contentViewId = annotationHelper.extractAnnotationFieldRefs(element, getTarget(),
                getEnvironment().getRClass().get(IRClass.Res.LAYOUT), false).get(0);
        ...
        onViewChangedBody
                .add(holder.getContextRef().invoke("getWindow").invoke("setFeatureInt").arg(customTitleFeature).arg(contentViewId));
    }
}
