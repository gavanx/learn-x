...
public abstract class AbstractFragmentByHandler extends CoreBaseAnnotationHandler<EComponentWithViewSupportHolder> implements
        MethodInjectionHandler<EComponentWithViewSupportHolder> {
    ...
    public AbstractFragmentByHandler(Class<? extends Annotation> targetClass, AndroidAnnotationsEnvironment environment,
            String findFragmentMethodName) {
    ...
    @Override
    public void assignValue(JBlock targetBlock, IJAssignmentTarget fieldRef, EComponentWithViewSupportHolder holder, Element element,
            Element param) {
        ...
        if (holder instanceof EFragmentHolder) {
            ...
            targetBlock.add(fieldRef.assign(cast(getJClass(typeQualifiedName), invoke(fragmentManagerGetter).invoke(findFragmentMethodName)
                    .arg(getFragmentId(element, fieldName)))));
        } else {
            ...
            targetBlock.add(fieldRef.assign(cast(getJClass(typeQualifiedName),
                    invoke(findFragmentMethod).arg(getFragmentId(element, fieldName)))));
        }
    }
