...
public class OnActivityResultHandler extends BaseAnnotationHandler<HasOnActivityResult> implements
        HasParameterHandlers<HasOnActivityResult> {
    ...
    public void process(Element element, HasOnActivityResult holder) throws Exception {
        ...
        for (VariableElement parameter : parameters) {
            ...
            if (parameter.getAnnotation(OnActivityResult.Extra.class) != null) {
                if (extras == null) {
                    extras = onResultBlock.decl(getClasses().BUNDLE, "extras_", JOp.cond(
                            intent.ne(_null()).cand(intent.invoke("getExtras").ne(_null())), intent.invoke("getExtras"),
                            _new(getClasses().BUNDLE)));
                }
    ...
    private static class ExtraHandler extends ExtraParameterHandler {
        ...
        public IJExpression getExtraValue(VariableElement parameter, JVar extras, JBlock block, HasOnActivityResult holder) {
            return super.getExtraValue(parameter, holder.getOnActivityResultDataParam(), extras, block, holder.getOnActivityResultMethod(),
                    holder);
        }
    }
