...
public class ModelExtractor {
     ...
     */
    public AnnotationElementsHolder extract(Set<? extends TypeElement> annotations, Set<String> annotationTypesToCheck,
    ...
    /**
     * Adds all root elements that are type element, and adds their enclosing type if they are not type elements (for annotated
     * elements such as fields and methods).
     */
    ...
    private void extractAncestorsAnnotations(AnnotationElementsHolder extractedModel, Set<String> annotationTypesToCheck,
            Set<TypeElement> rootTypeElements) {
    ...
    private void extractAnnotations(AnnotationElementsHolder extractedModel, Set<String> annotationTypesToCheck,
            TypeElement rootTypeElement, Element ancestorEnclosedElement) {
        ...
        for (AnnotationMirror annotationMirror : ancestorEnclosedElementAnnotations) {
            ...
            if (annotationTypesToCheck.contains(annotationType.toString())) {
                ...
                /*
                 * rootTypeElement is one of the types that are being compiled ancestorEnclosedElement is the annotated element in
                 * an ancestor of rootTypeElement annotation is a type representing the annotation on ancestorEnclosedElement
                 */

                extractedModel.putAncestorAnnotatedElement(annotation.getQualifiedName().toString(), ancestorEnclosedElement,
                        rootTypeElement);
            }
        }
    }
    ...
    private void extractRootElementsAnnotations(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv,
            AnnotationElementsHolder extractedModel) {
        for (TypeElement annotation : annotations) {
            extractedModel
                    .putRootAnnotatedElements(annotation.getQualifiedName().toString(), roundEnv.getElementsAnnotatedWith(annotation));
        }
    }
