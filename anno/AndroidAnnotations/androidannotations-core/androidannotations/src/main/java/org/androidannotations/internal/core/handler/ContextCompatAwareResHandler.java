...
abstract class ContextCompatAwareResHandler extends AbstractResHandler {
    ...
    ContextCompatAwareResHandler(AndroidRes androidRes, AndroidAnnotationsEnvironment environment, int minSdkWithMethod,
            String minSdkPlatformName) {
    ...
    private IJExpression createCallWithIfGuard(EComponentHolder holder, JFieldRef idRef, IJAssignmentTarget fieldRef, JBlock targetBlock) {
        ...
        IJExpression buildVersionCondition = getClasses().BUILD_VERSION.staticRef("SDK_INT").gte(
                getClasses().BUILD_VERSION_CODES.staticRef(minSdkPlatformName));
