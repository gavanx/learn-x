...
public class BitmapCache {
    ...
    private void init(ImageCacheParams cacheParams) {
        ...
        if (mCacheParams.memoryCacheEnabled) {
            //是否立即回收内存
            if (mCacheParams.recycleImmediately) mMemoryCache = new SoftMemoryCacheImpl(mCacheParams.memCacheSize);
            else mMemoryCache = new BaseMemoryCacheImpl(mCacheParams.memCacheSize);
        }
        // 是否启用sdcard缓存
    ...
    public Bitmap getBitmapFromMemoryCache(String data) {
        if (mMemoryCache != null) return mMemoryCache.get(data);
        return null;
    }
    ...
    public void clearDiskCache() {
        if (mDiskCache != null) mDiskCache.delete();
    }
