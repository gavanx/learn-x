...
/**
 * Handles loading/finding of modules, injection bindings, and static injections by use of a strategy of
 * "load the appropriate generated code" or, if no such code is found, create a reflective equivalent.
 */
public final class FailoverLoader extends Loader {
    /*
     * Note that String.concat is used throughout this code because it is the most efficient way to concatenate _two_
     * strings. javac uses StringBuilder for the + operator and it has proven to be wasteful in terms of both CPU and
     * memory allocated.
     */
    ...
    private final Memoizer<Class<?>, ModuleAdapter<?>> loadedAdapters = new Memoizer<Class<?>, ModuleAdapter<?>>() {
        ...
        protected ModuleAdapter<?> create(Class<?> type) {
            ModuleAdapter<?> result = instantiate(type.getName().concat(MODULE_ADAPTER_SUFFIX), type.getClassLoader());
            if (result == null) {
    ...
    public StaticInjection getStaticInjection(Class<?> injectedClass) {
        StaticInjection result = instantiate(injectedClass.getName().concat(STATIC_INJECTION_SUFFIX), injectedClass.getClassLoader());
        if (result != null) {
            return result;
        }
