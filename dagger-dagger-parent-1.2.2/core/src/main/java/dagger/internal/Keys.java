...
/**
 * Formats strings that identify the value to be injected. Keys are of one of three forms:
 * <ol>
 * <li>{@code com.square.Foo}: provides instances of Foo.
 * <li>{@code @com.square.Bar/com.square.Foo}: provides instances of Foo qualified by the annotation.
 * <li>{@code members/com.square.Foo}: injects members of Foo.
 * </ol>
 * Bindings from {@code @Provides} methods are of the first two types. BindingsGroup created from {@code @Inject}
 * -annotated members of a class are of the first and last types.
 */
public final class Keys {
    ...
    /**
     * Returns a key for {@code type} annotated with {@code annotations}, wrapped by {@code Set}, reporting failures
     * against {@code subject}.
     * 
     * @param annotations the annotations on a single method, field or parameter. This array may contain at most one
    ...
    /**
     * Validates that among {@code annotations} there exists only one annotation which is, itself qualified by
     * {@code \@Qualifier}
     */
    ...
    /**
     * @param topLevel true if this is a top-level type where primitive types like 'int' are forbidden. Recursive calls
     *            pass 'false' to support arrays like {@code int[]}.
     */
    ...
    /**
     * Returns a key for the type provided by, or injected by this key. For example, if this is a key for a
     * {@code Provider<Foo>}, this returns the key for {@code Foo}. This retains annotations and supports both Provider
    ...
    /**
     * Returns a key for the underlying binding of a Lazy<T> value. For example, if this is a key for a
     * {@code Lazy<Foo>}, this returns the key for {@code Foo}. This retains annotations.
     */
    ...
    /**
     * Returns an unwrapped key (the key for T from a Provider<T> for example), removing all wrapping key information,
     * but preserving annotations or known prefixes.
     * 
     * @param key the key from which the delegate key should be extracted.
     * @param start an index into the key representing the key's "real" start after any annotations.
     * @param delegatePrefix key prefix elements extracted from the underlying delegate (annotations, "members/", etc.)
    ...
    /**
     * Returns the class name for {@code key}, if {@code key} was created with a class instance. Returns null if
     * {@code key} represents a parameterized type or an array type.
     */
    public static String getClassName(String key) {
        ...
        return (key.indexOf('<', start) == -1 && key.indexOf('[', start) == -1) ? key.substring(start) : null;
    }
