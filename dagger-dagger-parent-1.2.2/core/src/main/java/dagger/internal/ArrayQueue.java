...
/**
 * Resizable-array implementation of the {@link Queue} interface. Array queues have no capacity restrictions; they grow
 ...
 * <p>
 * Most <tt>ArrayBackedQueue</tt> operations run in amortized constant time. Exceptions include {@link #remove(Object)
 * remove}, {@link #removeFirstOccurrence removeFirstOccurrence}, {@link #contains contains}, {@link #iterator
 * iterator.remove()}, and the bulk operations, all of which run in linear time.
 * <p>
 * The iterators returned by this class's <tt>iterator</tt> method are <i>fail-fast</i>: If the queue is modified at any
 ...
 */
public class ArrayQueue<E> extends AbstractCollection<E> implements Queue<E>, Cloneable, java.io.Serializable {
    /**
     * The array in which the elements of the queue are stored. The capacity of the queue is the length of this array,
     ...
     * wrapping around to equal each other. We also guarantee that all array cells not holding queue elements are always
     * null.
     */
    private transient Object[] elements;

    /**
     * The index of the element at the head of the queue (which is the element that would be removed by remove() or
     * pop()); or an arbitrary number equal to tail if the queue is empty.
     */
    private transient int head;

    /**
     * The index at which the next element would be added to the tail of the queue (via addLast(E), add(E), or push(E)).
     */
    private transient int tail;

    /**
     * The minimum capacity that we'll use for a newly created queue. Must be a power of 2.
     */
    ...
    /**
     * Double the capacity of this queue. Call only when full, i.e., when head and tail have wrapped around to become
     * equal.
     */
    ...
    /**
     * Constructs a queue containing the elements of the specified collection, in the order they are returned by the
     * collection's iterator. (The first element returned by the collection's iterator becomes the first element, or
     * <i>front</i> of the queue.)
     * 
     ...
     * <p>
     * This method is equivalent to {@link #offer}.
     * 
    ...
    /**
     * Retrieves and removes the head of the queue represented by this queue. This method differs from {@link #poll
     ...
     * 
     * @return the head of the queue represented by this queue, or <tt>null</tt> if this queue is empty
     */
    @Override
    public E poll() {
        int h = head;
    ...
    /**
     * Retrieves, but does not remove, the head of the queue represented by this queue. This method differs from
    ...
    /**
     * Retrieves, but does not remove, the head of the queue represented by this queue, or returns <tt>null</tt> if this
     * queue is empty.
     * 
     * @return the head of the queue represented by this queue, or <tt>null</tt> if this queue is empty
     */
    ...
    /**
     * Returns an iterator over the elements in this queue. The elements will be ordered from first (head) to last
     * (tail). This is the same order that elements would be queueued (via successive calls to {@link #remove} or popped
    ...
    /**
     * Removes a single instance of the specified element from this queue. If the queue does not contain the element, it
     * is unchanged. More formally, removes the first element <tt>e</tt> such that <tt>o.equals(e)</tt> (if such an
     * element exists). Returns <tt>true</tt> if this queue contained the specified element (or equivalently, if this
    ...
    /**
     * Returns an array containing all of the elements in this queue in proper sequence (from first to last element);
     * the runtime type of the returned array is that of the specified array. If the queue fits in the specified array,
    ...
    public ArrayQueue<E> clone() {
        try {
            ...
            E[] newElements = (E[]) Array.newInstance(elements.getClass().getComponentType(), elements.length);
     ...
     * 
     * @serialData The current size (<tt>int</tt>) of the queue, followed by all of its elements (each an object
