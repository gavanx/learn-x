...
final class ProvisionListenerStackCallback<T> {
    ...
    private static final ProvisionListenerStackCallback<?> EMPTY_CALLBACK = new ProvisionListenerStackCallback(null /* unused, so ok */, ImmutableList.of());
    ...
    public T provision(Errors errors, InternalContext context, ProvisionCallback<T> callable) throws ErrorsException {
        ...
        if (provision.exceptionDuringProvision != null) {
            ...
            throw errors.errorInUserCode(caught, "Error notifying ProvisionListener %s of %s.%n" + " Reason: %s", listener, binding.getKey(), caught).toException();
        } else {
