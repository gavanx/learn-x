...
public abstract class ProviderMethod<T> implements ProviderWithExtensionVisitor<T>, HasDependencies, ProvidesMethodBinding<T> {
    ...
    /*if[AOP]*/
        if (!skipFastClassGeneration && !Modifier.isPrivate(modifiers) && !Modifier.isProtected(modifiers)) {
            try {
                // We use an index instead of FastMethod to save a stack frame.
                return new FastClassProviderMethod<T>(key, method, instance, dependencies, parameterProviders, scopeAnnotation, annotation);
    ...
    public T get() {
        ...
        try {
            @SuppressWarnings({"unchecked", "UnnecessaryLocalVariable"}) T result = (T) doProvision(parameters);
            return result;
     ...
     */
    abstract Object doProvision(Object[] parameters) throws IllegalAccessException, InvocationTargetException;
    ...
    public boolean equals(Object obj) {
        if (obj instanceof ProviderMethod) {
            ...
            return method.equals(o.method) && instance.equals(o.instance) && annotation.equals(o.annotation);
        } else {
            return false;
        }
    }
  ...
  /*if[AOP]*/
    ...
    private static final class FastClassProviderMethod<T> extends ProviderMethod<T> {
        ...
        FastClassProviderMethod(Key<T> key, Method method, Object instance, ImmutableSet<Dependency<?>> dependencies, List<Provider<?>> parameterProviders, Class<? extends Annotation> scopeAnnotation, Annotation annotation) {
            ...
            this.fastClass = BytecodeGen.newFastClass(method.getDeclaringClass(), Visibility.forMember(method));
            ...
            this.methodIndex = fastClass.getIndex(new net.sf.cglib.core.Signature(method.getName(), org.objectweb.asm.Type.getMethodDescriptor(method)));
            Preconditions.checkArgument(this.methodIndex >= 0, "Could not find method %s in fast class for class %s", method, method.getDeclaringClass());
        }
