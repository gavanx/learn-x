    ...
    public <T> Boolean visit(Binding<T> command) {
        ...
        return command.acceptTargetVisitor(new Processor<T, Boolean>((BindingImpl<T>) command) {
            ...
            public Boolean visit(ConstructorBinding<? extends T> binding) {
                prepareBinding();
                try {
                    ConstructorBindingImpl<T> onInjector = ConstructorBindingImpl.create(injector, key, binding.getConstructor(), source, scoping, errors, false, false);
            ...
            public Boolean visit(InstanceBinding<? extends T> binding) {
                ...
                @SuppressWarnings("unchecked") // safe to cast to binding<T> because
                        ...
                        Initializable<T> ref = initializer.requestInjection(injector, instance, (Binding<T>) binding, source, injectionPoints);
            ...
            public Boolean visit(ProviderInstanceBinding<? extends T> binding) {
                ...
                Set<InjectionPoint> injectionPoints = binding.getInjectionPoints();
                Initializable<? extends javax.inject.Provider<? extends T>> initializable = initializer.<javax.inject.Provider<? extends T>>requestInjection(injector, provider, null, source, injectionPoints);
                // always visited with Binding<T>
                @SuppressWarnings("unchecked") InternalFactory<T> factory = new InternalFactoryToInitializableAdapter<T>(initializable, source, injector.provisionListenerStore.get((ProviderInstanceBinding<T>) binding));
                InternalFactory<? extends T> scopedFactory = Scoping.scope(key, injector, factory, source, scoping);
                putBinding(new ProviderInstanceBindingImpl<T>(injector, key, source, scopedFactory, scoping, provider, injectionPoints));
                return true;
            }
            ...
            public Boolean visit(ProviderKeyBinding<? extends T> binding) {
                ...
                // always visited with Binding<T>
                @SuppressWarnings("unchecked") BoundProviderFactory<T> boundProviderFactory = new BoundProviderFactory<T>(injector, providerKey, source, injector.provisionListenerStore.get((ProviderKeyBinding<T>) binding));
                bindingData.addCreationListener(boundProviderFactory);
                InternalFactory<? extends T> scopedFactory = Scoping.scope(key, injector, (InternalFactory<? extends T>) boundProviderFactory, source, scoping);
