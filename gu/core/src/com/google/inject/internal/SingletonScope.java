...
public class SingletonScope implements Scope {
     ...
     */
    private static final CycleDetectingLockFactory<Key<?>> cycleDetectingLockFactory = new CycleDetectingLockFactory<Key<?>>();
        ...
        return new Provider<T>() {
            ...
            public T get() {
                ...
                if (initialInstance == null) {
                    ...
                    final ListMultimap<Long, Key<?>> locksCycle = creationLock.lockOrDetectPotentialLocksCycle();
                    if (locksCycle.isEmpty()) {
                        ...
                        synchronized (constructionContext) {
                            ...
                            if (instance == null) {
                                ...
                                Dependency<?> dependency = Preconditions.checkNotNull(internalContext.getDependency(), "globalInternalContext.get(currentThread()).getDependency()");
                                Class<?> rawType = dependency.getKey().getTypeLiteral().getRawType();

                                try {
                                    @SuppressWarnings("unchecked") T proxy = (T) constructionContext.createProxy(new Errors(), internalContext.getInjectorOptions(), rawType);
                                    return proxy;
                                    ...
                                    Message cycleDependenciesMessage = createCycleDependenciesMessage(ImmutableMap.copyOf(globalInternalContext), locksCycle, exceptionErrorMessages.get(0));
