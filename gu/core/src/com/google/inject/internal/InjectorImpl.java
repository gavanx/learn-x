...
final class InjectorImpl implements Injector, Lookups {
    ...
    public <T> BindingImpl<T> getExistingBinding(Key<T> key) {
        ...
        synchronized (state.lock()) {
            ...
            for (InjectorImpl injector = this; injector != null; injector = injector.parent) {
                @SuppressWarnings("unchecked") BindingImpl<T> jitBinding = (BindingImpl<T>) injector.jitBindings.get(key);
                if (jitBinding != null) {
                    return jitBinding;
                }
            }
        ...
        if (isProvider(key)) {
            try {
                ...
                @SuppressWarnings({"unchecked", "cast"}) Key<?> providedKey = (Key<?>) getProvidedKey((Key) key, new Errors());
    ...
    <T> BindingImpl<T> createProvidedByBinding(Key<T> key, Scoping scoping, ProvidedBy providedBy, Errors errors) throws ErrorsException {
        ...
        @SuppressWarnings("unchecked") Key<? extends Provider<T>> providerKey = (Key<? extends Provider<T>>) Key.get(providerType);
